defmodule CreatorsPortalWeb.VideoOperationLiveTest do
  ...
  describe "disconnected and connected render" do
    ...
    test "video operation failed", %{conn: conn} do
      ...
      {:ok, video_live, disconnected_html} = live(Plug.Conn.assign(conn, :foo, :bar), Routes.video_operation_path(conn, :show, video_operation.trace_id, mode: "manual"))
  ...
  describe "handle tick" do
    test "video operation pending", %{conn: conn} do
      {:ok, video_live, _html} = live(conn, Routes.video_operation_path(conn, :show, "7682F3EB-65DF-4ABF-87E3-0F1143A67457", mode: "manual"))
